...
def create_intro(is_short):
    ...
    txt = TextClip("हिंदी मस्ती राइम्स 🦁 2026", fontsize=95, color='white', font=FONT_FILE if os.path.exists(FONT_FILE) else 'Arial', stroke_color='black', stroke_width=7).set_position('center').set_duration(2.2).crossfadein(0.8)
    return CompositeVideoClip([clip, txt], size=(w, h))

def create_outro(is_short):
    w, h = (1080, 1920) if is_short else (1920, 1080)
    clip = ColorClip((w, h), (20, 20, 40)).set_duration(4.5)
    txt1 = TextClip("LIKE 👍 SUBSCRIBE 🦁 SHARE", fontsize=92, color='#FFFF00', font=FONT_FILE if os.path.exists(FONT_FILE) else 'Arial', stroke_color='black', stroke_width=8).set_position(('center', h//3)).set_duration(4.5)
...
def make_video(content, is_short=True):
    print(f"🎥 Premium 2026 Render {'SHORT' if is_short else 'LONG'}...")
...
#HindiRhymes #BalGeet #NurseryRhymesForKids"""
        ...
        print(f"✅ UPLOADED WITH MAX SEO! {title}")
...
if __name__ == "__main__":
    ...
    print("🎉 Daily broadcast finished – Views will explode!")
