...
def save_to_memory(f, item):
    ...
    if item not in data:
        ...
        json.dump(
            data[-1000:],
            open(os.path.join(MEMORY_DIR, f), 'w', encoding='utf-8'),
...
def generate_content(mode="short"):
    ...
    themes = [
        "Outer Space Planets", "Jungle Safari Animals", "Underwater Ocean Fish",
        "Magic Trains and Cars", "Dinosaur Friends", "Talking Fruits", "Superheroes",
        "Construction Trucks", "Flying Vehicles", "Robot Pets", "Brave Birds",
        "Snowy Penguins", "Farm Animals"
    ]
    ...
    topic_prompt = (
        f"Output ONLY a 3-to-4 word English topic for a Hindi kids rhyme about: {theme}. "
        f"CRITICAL: Do NOT use rabbits, bunnies, cakes, cupcakes, or sweets. "
        f"Avoid: {', '.join(used[-20:])}."
    )
...
2. Pure Devanagari Hindi ONLY.
3. PERFECT RHYTHM: Exactly 5 to 7 words per line. AABB rhyme scheme.

CRITICAL VISUAL RULES:
5. Design a specific protagonist based on: [{archetype}]. Describe clothes, colors, features in exactly 10-15 English words.
6. EVERY 'image_prompt' MUST begin with the exact phrase from 'main_character'.
7. HOOK: Scene 1's image_prompt MUST describe the character doing something highly energetic (jumping, flying, cheering).

SEO TITLE RULE:
8. 'title' MUST format as: "English Title (Hindi Title) | 3D Hindi Rhymes | Bal Geet"
...
{{
  ...
  "scenes": [{{"line": "5 to 7 word Hindi sentence", "image_prompt": "Must start with main_character, followed by action."}}]
}}"""
    ...
    for attempt in range(4):
        ...
        if data and "scenes" in data and "main_character" in data:
            ...
            print(f"🎨 Unique Character: {data['main_character']}")
            return data
        time.sleep(4)
    return None
...
def get_image(image_prompt, fn, kw, is_short, video_seed):
    """
    Downloads image from Pollinations at native 1080x1920 (Shorts) or 1920x1080 (Long),
    then applies color grading for vibrancy. No upscaling crash risk.
    """
    ...
    # Force native resolution — no post-upscaling needed or wanted
    url = (
        f"https://pollinations.ai/p/{clean_encoded}"
        f"?width={w}&height={h}&seed={scene_seed}&model=flux&enhance=true&nologo=true"
    )
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # 🌟 UPGRADE 1: High-Fidelity Color Grading (safe, PIL-only, no crash risk)
...
def generate_text_clip_pil(
    text, w, h, base_size, dur,
    color='#FFFF00', pos_y=None, pos_x=None,
    stroke_width=8, is_english=False
):
    """
    🌟 UPGRADE 2: Anti-aliased Hindi subtitle rendering.
    Draws thick stroke + soft drop shadow for legibility on any background.
    """
    ...
    font = (
        ImageFont.truetype(target_font, size)
    ...
    for dx in range(-stroke_width, stroke_width + 1, 2):
        for dy in range(-stroke_width, stroke_width + 1, 2):
            draw.multiline_text(
                ...
                wrapped_text, font=font, fill=(0, 0, 0, 150),
                align="center"
            )
            draw.multiline_text(
                (x + dx, y + dy),
                wrapped_text, font=font, fill='black',
                align="center" if pos_x is None else "left"
            )

    # Main colored text on top
    draw.multiline_text(
        (x, y), wrapped_text, font=font, fill=color,
        align="center" if pos_x is None else "left"
    )
...
def create_outro(is_short):
    ...
    txt2 = generate_text_clip_pil(
        "@HindiMastiRhymes", w, h, 65, 4.0,
        color='#AAAAAA', pos_y=int(h * 0.55), is_english=True
    )
...
def create_segment_unified(line, img_path, is_short, idx, dur):
    """
    Uses MoviePy's native C++ resize/position lambdas for camera moves.
    These are memory-safe and GPU-accelerated — no custom numpy frame loops.
    """
    w, h = (1080, 1920) if is_short else (1920, 1080)
    ...
    wm = generate_text_clip_pil(
        "@HindiMastiRhymes", w, h, 38, dur,
        color='white', pos_y=40, pos_x=40,
        stroke_width=2, is_english=True
    )

    clip = CompositeVideoClip(
        [anim, txt, wm.set_opacity(0.60)], size=(w, h)
    ).set_duration(dur)

    if idx > 0:
        clip = clip.crossfadein(0.45)
    return clip
        ...
        try:
            subprocess.run(
                [
                    ...
                    "--rate=+5%",      # 🌟 TWEAKED: Faster, youthful rhythm
                    "--pitch=+15Hz",   # 🌟 TWEAKED: Higher, cute pitch
                    "--text", clean_speech,
                    "--write-media", fn
                ],
...
# ==========================================
# 🎬 VIDEO ASSEMBLY + BROADCAST ENCODE
# ==========================================
def make_video(content, is_short=True):
    ...
    print("🎨 Generating Images and Edge-TTS Voiceovers in parallel...")
    ...
    print("🎧 Mixing Studio Reverb + BGM...")
    for i, scene in enumerate(content['scenes']):
        aud = os.path.join(ASSETS_DIR, f"a_{suffix}_{i}.mp3")
        img = os.path.join(ASSETS_DIR, f"i_{suffix}_{i}.jpg")
        ...
        clip = create_segment_unified(
            scene['line'], img, is_short, i, dur
        ).set_audio(final_audio)
        clips.append(clip)

        times.append(
            f"{time.strftime('%M:%S', time.gmtime(current_time))} - {scene['line'][:55]}..."
        )
    ...
    # 🌟 UPGRADE 3: Broadcast Quality H.264 — CRF-only mode (no bitrate conflict)
    # preset='medium' = better compression than 'ultrafast' with no RAM crash risk
    # crf=19 = near-lossless, YouTube will re-encode at high quality
    # yuv420p = required for YouTube compatibility on all devices
    ...
    try:
        ...
        with Image.open(bg_path) as im:
            ...
            for off in [(6, 6), (8, 8)]:
                draw.multiline_text(
                    (x + off[0], 120 + off[1]),
                    wrapped_title, font=font_big, fill=(0, 0, 0), align="center"
                )
            draw.multiline_text((x, 120), wrapped_title, font=font_big, fill="#FFEA00", align="center")

            font_small = (
                ImageFont.truetype(ENG_FONT_FILE, 72)
                if os.path.exists(ENG_FONT_FILE)
                else ImageFont.load_default()
            )
...
def upload_video(vid, content, lyrics, times, desc_template, is_short):
    try:
        ...
        desc = (
            f"{title}\n\n"
            f"{desc_template.replace('[TIMESTAMPS]', chr(10).join(times))}\n\n"
            f"LIKE + SUBSCRIBE + SHARE for daily new rhymes!\n"
            f"#Shorts #HindiRhymes #BalGeet #KidsCartoon #{topic_tag}"
        )
        ...
        while response is None:
            try:
                ...
                if error_count > 5:
                    ...
                    return False
                print(f"⚠️ Connection dropped. Retrying {error_count}/5 in 5s...")
                time.sleep(5)
